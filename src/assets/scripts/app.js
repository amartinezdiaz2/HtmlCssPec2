...
//let detallesLibro = new URL("../../views/libros.html", import.meta.url);
...
for(let i = 0; i < 18; i ++){
    ...
    switch(libro.categoria){
        case 'Novela':
            novelasList.push(libro);
            break;
        case 'Biografía':
            biografiasList.push(libro);
            break;
        case 'Ensayo':
            ensayosList.push(libro);
            break;
        case 'Cuento':
            cuentosList.push(libro);
            break;
    }
}
...
if(window.location.href.includes("libros")){
    for (let i = 0; i < 18; i ++) {
        ...
        firstDiv.className = libro.categoria.replace('í', 'i');
    ...
    todos.onclick = function() {
        todos.classList.toggle('tras');
        novelas.classList.remove('tras');
        ensayos.classList.remove('tras');
        cuentos.classList.remove('tras');
        biografias.classList.remove('tras');
        boton('todos');
    }
    novelas.onclick = function() {
        todos.classList.remove('tras');
        novelas.classList.toggle('tras');
        ensayos.classList.remove('tras');
        cuentos.classList.remove('tras');
        biografias.classList.remove('tras');
        boton('Novela');
    ...
    biografias.onclick = function() {
        ...
        ensayos.classList.remove('tras');
        cuentos.classList.remove('tras');
        biografias.classList.toggle('tras');
        boton('Biografia');
    }
...
if(window.location.href.includes("librodetalle")){
    ...
    let informacion = document.getElementsByClassName('informacion')[0];
    // @ts-ignore
    informacion.firstElementChild.firstElementChild.src = libro.figure;

    let centro = document.getElementsByClassName('centro')[0];
    let centroChildren = centro.children;
    //Título
    centroChildren[0].innerHTML = libro.titulo;
    //Botón categoría
    centroChildren[1].innerHTML = libro.categoria;
    // @ts-ignore
    centroChildren[1].onclick = function() { navigation("../views/libros.html", id, libro.categoria.replace('í', 'i'))}
    
    //Resumen
    centroChildren[3].innerHTML = libro.resumen;

    //Fecha publicación
    centroChildren[5].innerHTML = libro.publicacion;

    let derecha = document.getElementsByClassName('derecha')[0];
    derecha.firstElementChild.innerHTML = otros(libro.categoria).titulo;
    ...
    for(let j = 0; j < listado.length; j ++){
        ...
        if(otroLibro.id !== libro.id){
            ...
            libroBoton.className = "libro-boton";
...
function boton(categoria){
    ...
    switch(categoria){
        case 'todos':
            ...
            displayCategory('Biografia', 'flex');
            break;
        case 'Novela':
            displayCategory('Novela', 'flex');
            displayCategory('Ensayo', 'none');
            displayCategory('Cuento', 'none');
            displayCategory('Biografia', 'none');
            break;
        case 'Ensayo':
...
function otros(categoria){
    switch(categoria){
        case 'Novela':
            return {titulo: "Otras novelas de la autora:", listado: novelasList};
        case 'Ensayo':
            return {titulo: "Otros ensayos de la autora:", listado: ensayosList};
        case 'Cuento':
            return {titulo: "Otros cuentos de la autora:", listado: cuentosList};
        case 'Biografía':
            return {titulo: "Otras biografías de la autora:", listado: biografiasList};
    }
}
...
**/
let novelasFooter = document.getElementById('novelas-footer');
let ensayosFooter = document.getElementById('ensayos-footer');
let cuentosFooter = document.getElementById('cuentos-footer');
let biografiasFooter = document.getElementById('biografias-footer');

list(novelasFooter, 'novelas');
list(ensayosFooter, 'ensayos');
list(cuentosFooter, 'cuentos');
list(biografiasFooter, 'biografias');

function list(element, classs) {
    let arrayElements = [];
    switch(classs){
        case 'novelas':
            arrayElements = novelasList;
            break;
        case 'cuentos':
            arrayElements = cuentosList;
            break;
        case 'biografias':
            arrayElements = biografiasList;
            break;
        case 'ensayos':
            arrayElements = ensayosList;
            break;
    };
    ...
    for(let i = 0; i < arrayElements.length; i ++){
        ...
        button.innerHTML = arrayElements[i].titulo;
        button.className = 'footer-button';
        button.onclick = function() {navigation('../views/librodetalle.html', arrayElements[i].id, classs)};
...
let botonFooter = document.getElementsByClassName('footer-list');
for(let i = 0; i < 5; i ++){
    // @ts-ignore
    botonFooter[i].onclick = function() { display(botonFooter[i]) }
}


function display(element) {
    let listVisibility = element.nextElementSibling.style.display;
